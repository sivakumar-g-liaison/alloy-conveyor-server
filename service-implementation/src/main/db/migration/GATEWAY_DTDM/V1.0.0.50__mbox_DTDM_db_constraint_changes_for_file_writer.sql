--
-- Copyright 2014 Liaison Technologies, Inc.
-- This software is the confidential and proprietary information of
-- Liaison Technologies, Inc. ("Confidential Information").  You shall
-- not disclose such Confidential Information and shall use it only in
-- accordance with the terms of the license agreement you entered into
-- with Liaison Technologies.
--

--Removing old folder constraint
DECLARE
	num_rows integer;
BEGIN
	SELECT count(*) INTO num_rows FROM USER_CONSTRAINTS WHERE table_name = 'FOLDER' AND constraint_name = 'CK01_FOLDER';

	IF num_rows = 1 THEN
		EXECUTE IMMEDIATE
			'ALTER TABLE GATEWAY_OWNR.FOLDER DROP CONSTRAINT CK01_FOLDER';
	END IF;
END;
/

--Removing old processor constraint
DECLARE
	num_rows integer;
BEGIN
	SELECT count(*) INTO num_rows FROM USER_CONSTRAINTS WHERE table_name = 'PROCESSOR' AND constraint_name = 'CK01_PROCESSOR';

	IF num_rows = 1 THEN
		EXECUTE IMMEDIATE 'ALTER TABLE GATEWAY_OWNR.PROCESSOR DROP CONSTRAINT CK01_PROCESSOR';
 	END IF;
END;
/

--Adding processor constraint
ALTER TABLE GATEWAY_OWNR.PROCESSOR
ADD CONSTRAINT CK01_PROCESSOR CHECK (TYPE IN ('REMOTEDOWNLOADER', 'REMOTEUPLOADER', 'SWEEPER', 'HTTPASYNCPROCESSOR', 'HTTPSYNCPROCESSOR', 'FILEWRITER'));

--Adding folder constraint
ALTER TABLE GATEWAY_OWNR.FOLDER
ADD CONSTRAINT CK01_FOLDER CHECK (TYPE IN ('PAYLOAD_LOCATION', 'RESPONSE_LOCATION', 'FILE_WRITE_LOCATION'));
