...
DECLARE
    ...
    v_c      int;
BEGIN
    SELECT count(*) INTO v_c FROM dba_tables WHERE upper(table_name) = upper('SCHEMA_VERSION') AND upper(owner) = upper('GANESH_TEST_OWNR');
    IF v_c = 0 THEN
        v_stmt := 'CREATE TABLE "GANESH_TEST_OWNR"."schema_version"
              (
        ...
        v_stmt := 'ALTER TABLE "GANESH_TEST_OWNR"."schema_version" ADD CONSTRAINT "schema_version_pk" PRIMARY KEY ("version") USING INDEX TABLESPACE "GANESH_TEST_CONF"';
        EXECUTE IMMEDIATE v_stmt;

        v_stmt := 'CREATE INDEX "GANESH_TEST_OWNR"."schema_version_s_idx" ON "GANESH_TEST_OWNR"."schema_version"("success") TABLESPACE "GANESH_TEST_CONF"';
        EXECUTE IMMEDIATE v_stmt;
        v_stmt := 'CREATE INDEX "GANESH_TEST_OWNR"."schema_version_ir_idx" ON "GANESH_TEST_OWNR"."schema_version" ("installed_rank") TABLESPACE "GANESH_TEST_CONF"';
        EXECUTE IMMEDIATE v_stmt;
        v_stmt := 'CREATE INDEX "GANESH_TEST_OWNR"."schema_version_vr_idx" ON "GANESH_TEST_OWNR"."schema_version" ("version_rank") TABLESPACE "GANESH_TEST_CONF"';
        EXECUTE IMMEDIATE v_stmt;
    END IF;
END;
/
