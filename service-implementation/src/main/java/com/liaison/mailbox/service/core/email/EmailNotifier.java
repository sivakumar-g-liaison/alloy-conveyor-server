...
public class EmailNotifier {
	 ...
	 * Send a single email.
	 */
	public void sendEmail(GenericEmailInfoDTO emailInfoDTO) {
		...
		try {
			...
			message.addHeader("Content-Type", emailInfoDTO.getType());
			message.setFrom(new InternetAddress(MAILSERVER_CONFIG.getProperty("mail.from")));

			List<String> toEmailAddrList = emailInfoDTO.getToEmailAddrList();
			...
			StringBuilder emailContentBuilder = new StringBuilder()
												.append(getEmailBodyPrefix(emailInfoDTO))
												.append(emailInfoDTO.getEmailBody())
												.append(NEW_LINE)
												.append(NOTE);
	 ...
	 */
	private String getEmailBodyPrefix(GenericEmailInfoDTO emailInfoDTO) {
		...
		try {
			...
			LOGGER.info("Unknown Host while constructing subject of email", e);
		}
		StringBuilder builder = new StringBuilder();

		// set mailbox name in email body if mailbox name is available
		if (!MailBoxUtil.isEmpty(emailInfoDTO.getMailboxName())) {
			builder.append(MAILBOX_NAME).append(SEPARATOR).append(emailInfoDTO.getMailboxName()).append(NEW_LINE);
		}

		// set mailbox id in email body if mailbox id is available
		if (!MailBoxUtil.isEmpty(emailInfoDTO.getMailboxId())) {
			builder.append(MAILBOX_ID).append(SEPARATOR).append(emailInfoDTO.getMailboxId()).append(NEW_LINE);
		}

		// set processor name in email body if processor name is available
		if (!MailBoxUtil.isEmpty(emailInfoDTO.getProcessorName())) {
			builder.append(PROCESSOR_NAME).append(SEPARATOR).append(emailInfoDTO.getProcessorName()).append(NEW_LINE);
		}
