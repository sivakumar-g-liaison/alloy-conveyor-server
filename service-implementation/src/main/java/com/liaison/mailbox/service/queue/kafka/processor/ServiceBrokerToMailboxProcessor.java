...
/**
 * ServiceBrokerToMailbox processor for Consumer usage
 *
 */
public class ServiceBrokerToMailboxProcessor implements KafkaTextMessageProcessor {
    ...
    @Inject
    public ServiceBrokerToMailboxProcessor(StatusLoggerFactory statusLoggerFactory) {
        this.statusLoggerFactory = statusLoggerFactory;
    }

    @Override
    public void processMessage(String message, String topic) {
        AsyncProcessThreadPool.getExecutorService().submit(new MailBoxService(message, MailBoxService.QueueMessageType.WORKTICKET));
    }
