...
public class MailboxTenancyKeyResource extends AuditedResource {
	...
	private static final StatsTimer statsTimer = new StatsTimer(
            MonitorConfig.builder("MailboxTenancyKeyResource_statsTimer").build(),
	...
	protected void beginMetricsCollection() {
        ...
        logKPIMetric(serviceCount, "MailboxTenancyKeyResource_serviceCallCounter");
	}
	...
	protected void endMetricsCollection(boolean success) {
        ...
        logKPIMetric(globalStatsTimer.getTotalTime() + " elapsed ms/" + globalStatsTimer.getCount() + " hits",
                "Global_timer");
        logKPIMetric(statsTimer.getTotalTime() + " ms/" + statsTimer.getCount() + " hits", "MailboxTenancyKeyResource_timer");
        logKPIMetric(duration + " ms for hit " + statsTimer.getCount(), "MailboxTenancyKeyResource_timer");

        if (!success) {
            logKPIMetric(globalFailureCounter.addAndGet(1), "Global_failureCounter");
            logKPIMetric(failureCounter.addAndGet(1), "MailboxTenancyKeyResource_failureCounter");
        }
	}
