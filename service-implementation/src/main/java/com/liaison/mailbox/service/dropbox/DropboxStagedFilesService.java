	...
	public StagePayloadResponseDTO addStagedFile(StagePayloadRequestDTO request, GlassMessage glassMessage) {
		...
		try {
			...
			if (null != glassMessage) {
				glassMessage.logProcessingStatus(StatusType.SUCCESS, MailBoxConstants.FILE_STAGED_SUCCESSFULLY, null, MailBoxConstants.DROPBOX_PROCESSOR);
			...
			if (null != glassMessage) {
				glassMessage.logProcessingStatus(StatusType.ERROR, "File Stage Failed :" + e.getMessage(), null, MailBoxConstants.DROPBOX_PROCESSOR);
