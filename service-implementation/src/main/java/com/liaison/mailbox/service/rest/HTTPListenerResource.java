		...
		AbstractResourceDelegate<Object> worker = new AbstractResourceDelegate<Object>() {
			...
			public Object call() throws Exception {
				...
				try {
                    ...
                    // Log status running
                    glassMessage.logProcessingStatus(StatusType.RUNNING, "HTTP Sync Request received", null, MailBoxConstants.HTTPSYNCPROCESSOR);
                    ...
                    if (syncResponse.getStatus() > 299) {
                        glassMessage.logProcessingStatus(StatusType.ERROR, "HTTP Sync Request failed: " + syncResponse.getEntity(), null, MailBoxConstants.HTTPSYNCPROCESSOR);
                    } else {
                        GlassMessage successMessage = constructGlassMessage(request, workTicket, mailboxPguid, ExecutionState.COMPLETED);
                        successMessage.logProcessingStatus(StatusType.SUCCESS, "HTTP Sync Request success", null, MailBoxConstants.HTTPSYNCPROCESSOR);
					...
					if (!Messages.PAYLOAD_ALREADY_EXISTS.value().equals(errorMessage)
	                    ...
	                    // Log error status
				        failedMsg.logProcessingStatus(StatusType.ERROR, "HTTP Sync Request Failed: " + e.getMessage(), null, MailBoxConstants.HTTPSYNCPROCESSOR);
		...
		AbstractResourceDelegate<Object> worker = new AbstractResourceDelegate<Object>() {
			...
			public Object call() throws Exception {
				...
				try {
                    ...
                    glassMessage.logFirstCornerTimestamp(firstCornerTimeStamp);
                    // Log running status
                    glassMessage.logProcessingStatus(StatusType.RUNNING, "HTTP Async request success", null, MailBoxConstants.HTTPASYNCPROCESSOR);
                    // Log TVA status
                    ...
                    if (!Messages.PAYLOAD_ALREADY_EXISTS.value().equals(errorMessage)
                            && !Messages.PAYLOAD_PERSIST_ERROR.value().equals(errorMessage)) {
                        // Log error status
                        glassMessage.logProcessingStatus(StatusType.ERROR, "HTTP Async Request Failed: " + e.getMessage(), null, MailBoxConstants.HTTPASYNCPROCESSOR);
                        glassMessage.setStatus(ExecutionState.FAILED);
                        transactionVisibilityClient.logToGlass(glassMessage);
                        glassMessage.logFourthCornerTimestamp();
                    }
