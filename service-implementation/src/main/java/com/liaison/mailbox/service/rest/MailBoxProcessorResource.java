...
public class MailBoxProcessorResource extends AuditedResource {
	...
	private static final StatsTimer statsTimer = new StatsTimer(
            MonitorConfig.builder("MailBoxProcessorResource_statsTimer").build(),
	...
	protected void beginMetricsCollection() {
        ...
        logKPIMetric(serviceCount, "MailBoxProcessorResource_serviceCallCounter");
	}
	...
	protected void endMetricsCollection(boolean success) {
        ...
        logKPIMetric(globalStatsTimer.getTotalTime() + " elapsed ms/" + globalStatsTimer.getCount() + " hits",
                "Global_timer");
        logKPIMetric(statsTimer.getTotalTime() + " ms/" + statsTimer.getCount() + " hits", "MailBoxProcessorResource_timer");
        logKPIMetric(duration + " ms for hit " + statsTimer.getCount(), "MailBoxProcessorResource_timer");

        if (!success) {
            logKPIMetric(globalFailureCounter.addAndGet(1), "Global_failureCounter");
            logKPIMetric(failureCounter.addAndGet(1), "MailBoxProcessorResource_failureCounter");
        }
	}
