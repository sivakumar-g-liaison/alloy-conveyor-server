...
var process = function(_proc) {
    ...
    var processorStaticProperties = JSON.parse(_proc.getPropertiesJson());
    ...
    try {
        ...
        _proc.logInfo("use fie system - " + processorStaticProperties.useFileSystem);

        //----------------------------------------------------------------------------
        // PART 2 - Check if there are files to be processed. stop if not
        //----------------------------------------------------------------------------
        if (processorStaticProperties.useFileSystem) {
            fileArray = _proc.getFilesToUpload(processorStaticProperties.recurseSubDirectories);
        } else {
            fileArray = _proc.getRelayFiles(processorStaticProperties.recurseSubDirectories);
        }
    ...
    if (arrayLength > 0) {
        ...
        try {
            ...
            for (var i = 0; i < arrayLength; i++) {
                ...
                actualFile = fileArray[i];
                uploadFileToRemote(ftp_request, remoteTargetLocation, actualFile, _proc, MSGContext, mboxInfo, processorStaticProperties);
                _proc.logInfo(mboxInfo + localPayloadLocation.substr(11) + " " + actualFile.getName() + " " + String(i + 1) + " of " + String(arrayLength) + " Completed FTP for: " + actualFile + " " + MSGContext);
            }
...
//
var uploadFileToRemote = function(ftpRequest, remoteParentDir, _actualFile, _proc, _MSGContext, _mboxInfo, processorStaticProperties) {
    ...
    try {
        ...
        var useTriggerFileEx = String(customProperties.getProperty('use_trigger_file_extension'));
        _proc.logInfo(_mboxInfo + "use_trigger_file_extension is ::" + useTriggerFileEx);

        var replaceExtnWithTrigger = (String(customProperties.getProperty('replace_extension_with_trigger')) === "true");
        _proc.logInfo(_mboxInfo + "replace_extension_with_trigger is ::" + replaceExtnWithTrigger);

        /**
         * File name with progress indicator
         * @returns {string}
         */
        ...
        if (processorStaticProperties.useFileSystem) {
            inputStream = new java.io.FileInputStream(_actualFile);
        } else {
