...
var process = function(_proc) {
    ...
    try {
        ...
        mboxInfo = "mailbox:" + mboxName + " processor:" + prcsrName + " ftps_uploader_1.0 ::: ";
        ...
        _proc.sendEmail(null, "Failure", "ftp(s) uploader failed during initialization : " + MSGContext + " " + e, "HTML");
        throw e;
    }
    ...
    if (arrayLength > 0) {
        ...
        try {
            ...
            ftp_request.connect();
            ftp_request.enableDataChannelEncryption();
            ftp_request.enableSessionReuse(true);
            ftp_request.login();
            ...
            _proc.logError(mboxInfo + " exception " + e);
            _proc.sendEmail(null, "Failure", "ftp(s) uploader failed because: " + MSGContext + " " + e, "HTML");
...
var uploadFileToRemote = function(ftpRequest, remoteParentDir, _actualFile, _proc, _MSGContext, _mboxInfo) {
    ...
    try {
        ...
        //Use this when when getFilesToUpload
        ...
        if (!isEmpty(useTriggerFileEx)) {
            ...
            replyCode = ftpRequest.renameFile(fileNameWithProgressIndicatorStatus(), triggerFileName + useTriggerFileEx);
        }
